# argparse2
# Copyright @ 2025 Wasfi JAOUAD. All rights reserved.
# Contact : wj.osprojects@gmail.com
# Published under the Python License : https://github.com/python/cpython/blob/main/LICENSE
# Based on argparse from CPython (all rights reserved).
...
class HelpFormatter(object):
    ...
    def __init__(self,
        ...
        self._prog = prog
        self.short_indent = short_indent
    ...
    def _insert(self, text):
        self._current_section.items.append((lambda txt: txt, [text]))
    ...
    def add_text(self, text, raw=False):
        if text is not SUPPRESS and text is not None:
            self._add_item(self._format_text, [text,raw])

    def add_usage(self, usage, actions, groups, prefix=None):
        if usage is not SUPPRESS:
            args = usage, actions, groups, prefix
            # self._insert(self.nl)
            self._add_item(self._format_usage, args)

    def add_argument(self, action, short=False):
        if action.help is not SUPPRESS:
            ...
            if short : self._add_item(self._format_action_short, [action, self.short_indent])
            else :     self._add_item(self._format_action, [action])

    def add_arguments(self, actions, short=False):
        for action in actions:
            self.add_argument(action, short)
    ...
    def _indent_wrap(self, text, indent=0, width=None) :
        if not text : return ''
        if width == None : width=self._width
        text = text.strip() #% self._prog
        lines = []
        import textwrap; L = 0
        for lin in text.replace(self.NL,self.NL+self.NL).split(self.NL) :
            if not lin.strip() : lines.append(self.nl); continue
            if 0 <= (pos := lin.find(self.indent_pick)) :
                for line in textwrap.fill(self._whitespace_matcher.sub(' ', lin.replace(self.indent_pick,'')),
                   width = width-indent, initial_indent='%*s' % (indent,''),
                   subsequent_indent = '%*s' % (indent+pos+1,'')
                ).splitlines() :
                      lines.append('%s\n' % (line))
                continue
            for line in textwrap.wrap(self._whitespace_matcher.sub(' ', lin), 
              width=width-indent, drop_whitespace=False) :
                lline = line.lstrip()
                LL = len(line[:-len(lline)])
                lines.append('%*s%s\n' % (indent+LL,'', lline))
                if LL > 0 : indent = indent + LL - L; L=LL
                else : indent -= L
        return ''.join(lines)

    def _format_text(self, text, raw=False):
        if '%(prog)' in text:
            text = text % dict(prog=self._prog)
        text_width = max(self._width - self._current_indent, 11)
        indent = ' ' * self._current_indent
        return self._fill_text(text, text_width, indent, raw) + '\n\n'
    
    def _format_action_short(self, action,indent):
        # determine the required width and the entry label
        help_position = min(self._action_max_length + 2,
                            self._max_help_position)
        help_width = max(self._width - help_position, 11)
        action_width = help_position - indent - 2
        action_header = self._format_action_invocation_short(action)
        
        # no desc : add a final newline
        if not action.desc:
            tup = indent, '', action_header
            action_header = '%*s%s\n' % tup

        else :
            tup = indent, '', action_width, action_header
        ...
        for subaction in self._iter_indented_subactions(action):
            parts.append(self._format_action_short(subaction,indent))
    ...
    def _format_action(self, action):
        ...
        if action.help and action.help.strip():
            help_text = self._expand_text(action,'help')
            if help_text:
                help_lines = self._split_lines(help_text, help_width)
                parts.append('%*s%s\n' % (indent_first, '', help_lines[0]))
                for line in help_lines[1:]:
    ...
    def _format_action_invocation_short(self, action):
        if not action.option_strings:
            default = self._get_default_metavar_for_positional(action)
            return ' '.join(self._metavar_formatter(action, default)(1))
    ...
    def _expand_text(self, action, attr):
        ...
        if attr == 'help' : return self._get_help_string(action) % params
        return getattr(action,attr) % params
    ...
    def _fill_text(self, text, width, indent, raw=False):
        if raw :
...
class ArgumentDefaultsHelpFormatter(HelpFormatter):
    ...
    def _get_help_string(self, action):
        ...
        if( action.default != None and '%(default)' not in action.help
          and 'default '.casefold() not in action.help.casefold() ) :
            if type(action.default) != type(_sys.stdout) :
              if isinstance(action.default, list) : 
                     help += ' (default: '+str(action.default[0])+')'
              else : help += ' (default: %(default)s)'
            else :
              if   action.default == sys.stdout : help += ' (default: stdout)'
              elif action.default == sys.stderr : help += ' (default: stderr)'
              elif action.default == sys.stdin  : help += ' (default: stdin)'
        
        return help


class ArgumentQualifHelpFormatter(HelpFormatter):
    """Help message formatter which adds default values & qualification (recommeded, required) to argument help.
    ...
    def _get_help_string(self, action):
        ...
        if action.required :  help = 'Required. '    + help
        if action.recommend : help = 'Recommended. ' + help
...
class ArgumentError(Exception):
    ...
    def __str__(self):
        if self.argument_name is None:
            ...
            format = _('argument %(argument_name)s'+self.sep+'%(message)s')
...
class Action(_AttributeHolder):
    ...
    def __init__(self,
                 ...
                 dest,
                 nargs=None,
                 max_use_count=0, max_use_err=None,
                 const=None,
                 default=None,
                 type=None,
                 choices=None,
...
class BooleanOptionalAction(Action):
    def __init__(self,
        ...
        super().__init__(
            ...
            choices=choices,
            required=required,
            max_use_count=max_use_count, max_use_err=max_use_err,
            recommend=recommend,
            help=help, desc=desc,
            metavar=metavar,
...
class _ShortHelpAction(Action):
  ...
  def __init__(self,
               ...
               help=None, desc=None,
               deprecated=False):
    super(_ShortHelpAction, self).__init__(
          option_strings=option_strings,
          dest=dest,
          default=default,
          nargs=0,
...
class _ActionsContainer(object):
    ...
    def __init__(self,
        ...
        self.register('action', 'short_help', _ShortHelpAction)
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def __init__(self,
                 ...
                 add_help=True,
                 error_tag='Error',
                 width=None, NL='@NL@', indent_pick='@FI@',
                 allow_abbrev=True,
                 exit_on_error=True):
        ...
        if self.formatter_class is RawTextHelpFormatter : self.raw_description = True
        else : self.raw_description = raw_description
    ...
    def _parse_known_args(self, arg_strings, namespace, intermixed):
        ...
        option_counts = dict()
        lastOptString = dict(); arg_strings_count = len(arg_strings)
        def consume_optional(start_index):
            ...
            for action, args, option_string in action_tuples:
                opt1 = action.option_strings[0]
                if opt1 in option_counts : option_counts[opt1] += 1
                else : option_counts[opt1] = 1
                if hasattr(action,'max_use_count') and action.max_use_count > 0 :
                    lastOptStr = option_string
                    ...
                    if option_counts[opt1] > action.max_use_count :
                        ...
                        errmsg = ' :\n\n     '+lastOptString[opt1] \
                                   + '\n     '+lastOptStr \
                                   + errmsg
                        raise ArgumentError(action, _('used more than once%s') %errmsg, ' ')
                    lastOptString[opt1] = lastOptStr
    ...
    def format_help(self):
        ...
        # description
        formatter.add_text(self.description, self.raw_description)
    ...
    def format_help_short(self):
        formatter = self._get_formatter()
        formatter.add_usage(self.usage, self._actions, self._mutually_exclusive_groups,'  Usage : ')
        # formatter.add_text(self.description)
        for action_group in self._action_groups:
            formatter.add_arguments(action_group._group_actions, short=True)
        if self.example_use and self.example_use.strip().replace('\n','') != "" :
            formatter._insert(formatter.nl)
            formatter._add_item(formatter._indent_wrap, [self.example_use.strip('\n') % dict(prog=self.prog), 2])
        return formatter.format_help()
        
    def _get_formatter(self):
        return self.formatter_class(prog=self.prog, width=self.width, NL=self.NL, indent_pick=self.indent_pick)
    ...
    def print_short_help(self, file=None):
        if file is None:
            file = _sys.stdout
        self._print_message(self.format_help_short(), file)
